...
#Class Node for the linked list, init pointers 
class Node:
...
# creates a dictionary of Queues. Key = server channel name
queues = {}
x = "default"
queues[x] = Queue()
...
async def q(ctx):
    ...
    await message.add_reaction("⏭")
    await ctx.message.delete()
#NEED to update authorId with ur bots full discord id (a bunch of numbers)
@bot.event
async def on_message(message):
    global boop
    global beep
    elif "Press ✅ to join, ❌ to leave, or ⏭ to bump the queue!" in message.content and message.author.id == "YOUR BOTS ID":
        boop = message.id
...
#NEED to UPDATE AUTHOR WITH YOUR BOTS DISCORD ID 
    if (str(reaction.message.author) == "AUTHOR" and (
